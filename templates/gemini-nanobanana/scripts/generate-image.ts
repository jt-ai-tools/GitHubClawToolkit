...
function buildUserPrompt(params: {
  ...
  return [
    ...
    '"""',
    '',
    '請先整合系統指示、需求與上下文，輸出可直接用於影像生成的最終提示詞。',
  ].join('\n');
}
...
async function main(): Promise<void> {
  ...
  const model = normalizeText(process.env.VERTEXAI_NANOBANANA_MODEL || process.env.NANOBANANA_MODEL) || 'gemini-3-pro-image-preview';
  ...
  const response = await ai.models.generateContent({
    ...
    contents: [{
      role: 'user',
      parts: [{
        text: buildUserPrompt({ systemPrompt, userPrompt, contextJsonl }),
      }],
    }],
  });
  ...
  if (imageParts.length === 0) {
    throw new Error('Gemini AI response did not contain any image output.');
  }
