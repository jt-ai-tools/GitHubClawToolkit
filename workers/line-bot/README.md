...
bun install
bun run build
WORKER_NAME=your-worker-name bun run dev
WORKER_NAME=your-worker-name bun run set
WORKER_NAME=your-worker-name bun run deploy
bun run test
```
...
- 本地開發請把值放在 `.dev.vars`
- 部署到 Cloudflare 時可直接執行 `WORKER_NAME=your-worker-name bun run set`
